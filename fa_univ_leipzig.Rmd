...
```{r pca}
pca.univ<-princomp(scale(univ_num), corr=TRUE)
summary(pca.univ)
```

So the first component provides 42% of the variance, then 11%, then 8%.
...
To be honest this looks like one component to rule them all, but I took the first 3.
